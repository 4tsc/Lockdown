...
private fun BlockScreenContent(appLabel: String, onGoHome: () -> Unit) {
    ...
    Column(
        ...
        Text("Fuera del horario que configuraste para esta app.", style = MaterialTheme.typography.bodyMedium)
        Spacer(Modifier.height(32.dp))
        if (!showUnlock) {
            Button(onClick = onGoHome) { Text("Ir al inicio") }
            Spacer(Modifier.height(8.dp))
            OutlinedButton(onClick = { showUnlock = true }) { Text("Solicitar desbloqueo") }
        } else {
            com.example.lockdown.ui.common.UnlockCodeCard(onUnlocked = onGoHome)
        }
    }
}
